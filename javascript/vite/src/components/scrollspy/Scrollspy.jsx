...
const Scrollspy = ({
  ...
  const anchorElementsRef = useRef(null); // Store anchor elements in useRef to avoid rerenders
  ...
  useEffect(() => {
    ...
    if (selfRef.current) {
      anchorElementsRef.current = selfRef.current.querySelectorAll(`[data-${dataAttribute}-anchor]`);
    }
    anchorElementsRef.current?.forEach(item => {
