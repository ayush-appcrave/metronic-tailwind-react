...
const ScrollSpy = ({
  ...
  const [navContainerItems, setNavContainerItems] = useState(); // prettier-ignore

  const prevIdTracker = useRef('');
  useEffect(() => {
    navContainerRef ? setNavContainerItems(navContainerRef.current.querySelectorAll(`[data-${useDataAttribute}]`)) : setNavContainerItems(document.querySelectorAll(`[data-${useDataAttribute}]`));
  ...
  const isVisible = el => {
    ...
    if (useBoxMethod) {
      const useHeight = parentScrollContainerRef?.current ? parentScrollContainerRef.current.offsetHeight : window.innerHeight;
      ...
      const leniency = parentScrollContainerRef?.current ? parentScrollContainerRef.current.offsetHeight * 0.5 : window.innerHeight * 0.5;
      const useHeight = parentScrollContainerRef?.current ? parentScrollContainerRef?.current.offsetHeight : window.innerHeight;
    ...
    for (let i = 0; i < scrollParentContainer.children.length; i++) {
      ...
      if (elementIsVisible) {
        ...
        navContainerItems.forEach(el => {
          ...
          if (attrId === changeHighlightedItemId && !el.classList.contains(activeClass ?? 'active')) {
  ...
  useEffect(() => {
    parentScrollContainerRef ? parentScrollContainerRef.current.addEventListener('scroll', () => {
