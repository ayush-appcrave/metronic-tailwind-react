...
function TableBodyCustom(props) {
  ...
  return <TableBody sx={{
    position: 'relative'
      ...
      return <TableRow hover role="checkbox" aria-checked={isSelected(row.id)} tabIndex={-1} key={`enhanced-table-row-${row.id}`} selected={isSelected(row.id)}>
            <TableCell padding="checkbox">
              <Checkbox color="primary" size="small" onInput={event => {
            onSelect(row.id);
